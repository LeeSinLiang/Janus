...
def scenario_3_metrics_analysis_and_improvement(campaign: Campaign, product_info: str):
	...
	for line in analysis.analysis:
		print(line.analyzed_report[:500] + "\n...")
	print("-" * 80)
	...
	for i, post in enumerate(posts, 1):
		...
		old_content_a = old_variants.filter(variant_id='A').first()
		...
		improved_output = content_agent.execute_with_metrics(
			...
			analyzed_report=analysis.analysis
		)

		# Update existing variants or create new ones
		variant_a = old_variants.filter(variant_id='A').first()
		variant_b = old_variants.filter(variant_id='B').first()
